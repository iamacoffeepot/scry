...
mod tests {
    ...
    /// The validation game: a directory holding `match.json` and
    /// `timeline-events.jsonl` named by `SCRY_FIXTURE_DIR`, and the tracked
    /// player's PUUID in `SCRY_FIXTURE_PUUID`. Archives hold real players, so
    /// the fixture stays out of the tree; unset means the test skips.
    #[allow(clippy::disallowed_methods)] // test-only pointer at a local archive, not cap config
    fn fixture() -> Option<(Match, String, String)> {
        let dir = PathBuf::from(std::env::var("SCRY_FIXTURE_DIR").ok()?);
        let puuid = std::env::var("SCRY_FIXTURE_PUUID").ok()?;
    ...
    /// The headline: Red side (the player's team) won multiple early fights but
    ...
    fn conversion_gap_is_the_story() {
        let Some((game, events, puuid)) = fixture() else {
            eprintln!("fixture archive absent; skipping");
            return;
        };
        let moments = analyze(&game, &events, &puuid);
    ...
    /// The player died mostly for nothing, to one repeat killer, and touched no epics.
    #[test]
    fn fixture_death_and_objective_signals() {
        let Some((game, events, puuid)) = fixture() else {
            eprintln!("fixture archive absent; skipping");
            return;
        };
        ...
        assert!(
            ...
            "expected the player absent from all majors"
        );
        assert!(
