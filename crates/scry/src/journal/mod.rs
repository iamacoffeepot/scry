...
mod tests {
    ...
    fn journal_roundtrips_and_folds() {
        ...
        assert!(projection.is_posted("NA1", 100, "Faker#KR1"), "case-insensitive dedup");
        assert!(!projection.is_posted("NA1", 102, "Faker#KR1"));
        assert_eq!(projection.message_id("NA1", 101, "Chovy#KR1"), Some("m-101"));
        assert_eq!(projection.message_id("NA1", 101, "Faker#KR1"), None, "another player's post is not mine");
        ...
        assert_eq!(projection.newest_posted("Faker#KR1", Some(420)), Some(100));
        assert_eq!(projection.newest_posted("Faker#KR1", Some(440)), None);
        assert_eq!(projection.newest_posted("Chovy#KR1", None), Some(101));
        ...
        let picks = with_picks.picks("NA1", 101, "Chovy#KR1").expect("picks re-key through the puuid pin");
        assert_eq!(picks.highlight, Some(pick.clone()));
        assert_eq!(picks.lowlight, None);
        assert_eq!(pick.caption(), "**2:05** — the outplay");
        assert!(with_picks.picks("NA1", 100, "Faker#KR1").is_none());

        // Identity: a pin re-keys name-era rows to the PUUID, so after a
        // rename the NEW name still sees the old posts and floors.
        journal2.append(&AccountResolved { riot_id: "Faker#KR1".into(), puuid: "p-faker".into() }).unwrap();
        journal2.append(&AccountResolved { riot_id: "Faker#T1".into(), puuid: "p-faker".into() }).unwrap();
        let renamed = journal2.fold().unwrap();
        assert!(renamed.is_posted("NA1", 100, "Faker#T1"), "rename keeps dedup");
        assert_eq!(renamed.newest_posted("Faker#T1", Some(420)), Some(100), "rename keeps the floor");
        ...
        journal2.append(&AccountResolved { riot_id: "Chovy#KR1".into(), puuid: "p-chovy-rekeyed".into() }).unwrap();
        journal2.append(&AccountResolved { riot_id: "Faker#T1".into(), puuid: "p-faker-rekeyed".into() }).unwrap();
        let swapped = journal2.fold().unwrap();
        assert!(swapped.is_posted("NA1", 100, "Faker#T1"), "key swap keeps dedup");
        assert_eq!(swapped.newest_posted("Faker#T1", Some(420)), Some(100), "key swap keeps the floor");
        assert!(swapped.picks("NA1", 101, "Chovy#KR1").is_some(), "key swap keeps picks");
        ...
        journal2.append(&AccountResolved { riot_id: "Hideonbush#KR1".into(), puuid: "p-1".into() }).unwrap();
        journal2.append(&AccountResolved { riot_id: "Hideonbush#KR1".into(), puuid: "p-1-rekeyed".into() }).unwrap();
