...
//! the recorder (`scripts/highlight.sh`, or `highlight.ps1` on Windows)
//! handed each perspective's clip windows, edit the
...
async fn clips_pass(cli: &Cli, journal: &Journal) -> Result<()> {
    let mut recorded = 0;
    let projection = journal.fold()?;
    let client_patch = client_replay_patch(&cli.lol_lockfile).await;
...
async fn try_game_clips(
    ...
    match client_phase(&cli.lol_lockfile).await.as_deref() {
    ...
    if !replay_ready(&cli.lol_lockfile, game_id).await {
    ...
    let mut command = recorder_command();
    command.env("SCRY_LOL_LOCKFILE", &cli.lol_lockfile).arg(&dir);
    ...
    let status = command.status().await.context("running the clip recorder")?;
    if !status.success() {
...
/// The clip recorder for this platform: the bash script on macOS, its
/// PowerShell port on Windows. Both take `<archivedir> <spec>…` and read the
/// client lockfile from `SCRY_LOL_LOCKFILE`.
fn recorder_command() -> tokio::process::Command {
    if cfg!(windows) {
        let mut command = tokio::process::Command::new("powershell.exe");
        command.args(["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", r"scripts\highlight.ps1"]);
        command
    } else {
        tokio::process::Command::new("scripts/highlight.sh")
    }
}

/// One perspective's argument to the clip recorder:
...
async fn replay_ready(lockfile: &Path, game_id: u64) -> bool {
    let _ = lcu_post(lockfile, &format!("/lol-replays/v1/rofls/{game_id}/download/graceful"), "{}").await;
    for _ in 0..4 {
        tokio::time::sleep(std::time::Duration::from_secs(3)).await;
        if let Some(body) = lcu_get(lockfile, &format!("/lol-replays/v1/metadata/{game_id}")).await
...
pub(crate) async fn client_replay_patch(lockfile: &Path) -> Option<String> {
    let body = lcu_get(lockfile, "/lol-replays/v1/configuration").await?;
...
/// down/unreachable.
async fn client_phase(lockfile: &Path) -> Option<String> {
...
async fn lcu_post(lockfile: &Path, path: &str, body: &'static str) -> Option<String> {
    lcu_request(lockfile, path, Some(body)).await
}

/// GET one LCU endpoint through the lockfile-published local port; `None`
/// when the client is down or the request fails.
async fn lcu_get(lockfile: &Path, path: &str) -> Option<String> {
    lcu_request(lockfile, path, None).await
}

async fn lcu_request(lockfile: &Path, path: &str, post_body: Option<&'static str>) -> Option<String> {
    let lockfile = std::fs::read_to_string(lockfile).ok()?;
...
fn post_cli(cli: &Cli, riot_id: &str, dir: &Path, edit: bool) -> Cli {
    Cli {
        ...
        lol_lockfile: cli.lol_lockfile.clone(),
        count: 1,
        queue: None,
        dump: None,
...
/// Split `NA1_4242424242` into `("NA1", 4242424242)`.
...
mod tests {
    ...
    fn match_ids_split_into_platform_and_game() {
        assert_eq!(split_match_id("NA1_4242424242").unwrap(), ("NA1", 4242424242));
    ...
    fn slug_matches_the_shell_marker_names() {
        ...
        assert_eq!(slug("Faker#KR1"), "Faker_KR1");
