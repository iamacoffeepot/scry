...
pub struct Cli {
    ...
    /// Mark clip jobs abandoned for these match ids (e.g. NA1_4242424242):
    ...
    /// The League client's lockfile, which publishes the local client API's
    /// port and password. Defaults to the stock install location on this
    /// platform; point it elsewhere for a custom install.
    #[arg(long, env = "SCRY_LOL_LOCKFILE", default_value_os_t = default_lockfile())]
    pub lol_lockfile: PathBuf,
}

/// Where a stock League install keeps the client lockfile.
