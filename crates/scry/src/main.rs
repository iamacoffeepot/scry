...
async fn post_from_archive(cli: &Cli, dir: &Path) -> Result<()> {
    ...
    let clip_note = if highlight.is_none() && lowlight.is_none() {
        match (tick::game_patch(dir), tick::client_replay_patch(&cli.lol_lockfile).await) {
    ...
    // just uncaptioned).
    // A post straight from an archive gets its captions the way a tick post
    // does: journal the picks first when none are recorded yet.
    let state = if !cli.edit && state.picks(platform, game_id, riot_id).is_none() {
        journal_picks(&journal, riot_id, dir, &tick::roster_riot_ids(cli))?;
        journal.fold()?
    } else {
        state
    };
