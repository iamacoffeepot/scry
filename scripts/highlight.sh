...
#
# where <suffix> selects that player's output names (e.g. `-Faker_KR1`) and a
...
# with EnableReplayApi=1 in game.cfg.
...
dir="$(cd "$dir" && pwd)"
gid="$(basename "$dir")"        # numeric game id (e.g. 4242424242)

# PREROLL is just buffer: we seek a couple seconds before the clip start and
# let the replay stream in before recording.
PREROLL=3

# The client lockfile; the tick passes its --lol-lockfile through the env.
lockfile="${SCRY_LOL_LOCKFILE:-/Applications/League of Legends.app/Contents/LoL/lockfile}"
