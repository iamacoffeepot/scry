...
validated against an archived NA game (the tracked player on Ezreal
...
on the player's tower at ~5:00 (plate at 5:35 confirms the champ-position read), oscillates
as both ADCs trade backs, the player's side lands counter-crashes (plates 9:21, 13:40),
...
- **Fired:** Red (the player's team) **won 4 of its first 5 skirmishes and converted only
...
- **Fired:** all **4 dragons by one Graves** → Soul + monopoly; Baron 28:40 Blue.
  The player was **never within participation of a single dragon or Baron** — for a bot
...
- **Fired:** **Akshan killed the player in 6 of 10 deaths**, 4 of them free. Composes with
...
Validation harness: the archived validation game (`SCRY_FIXTURE_DIR` / `SCRY_FIXTURE_PUUID` for the tests) is the golden fixture — every join's
